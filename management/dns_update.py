	...
	for qname, rtype, value in custom_dns:
		...
		for hostname in value.split(" "):
			...
			if not hostname.startswith("xfr:"):
				if mode == "xfr":
					response = dns.resolver.resolve(hostname+'.', "A", raise_on_no_answer=False, search=True)
					values.extend(map(str, response))
					response = dns.resolver.resolve(hostname+'.', "AAAA", raise_on_no_answer=False, search=True)
					values.extend(map(str, response))
					continue
				values.append(hostname)
	...
	if len(hostnames) > 0:
		...
		for item in hostnames:
			if not item.startswith("xfr:"):
				# Resolve hostname.
				try:
					response = resolver.resolve(item, "A", search=True)
				except (dns.resolver.NoNameservers, dns.resolver.NXDOMAIN, dns.resolver.NoAnswer):
					try:
						response = resolver.resolve(item, "AAAA", search=True)
					except (dns.resolver.NoNameservers, dns.resolver.NXDOMAIN, dns.resolver.NoAnswer):
